  ...
  useEffect(() => {
    const fetchAndPopulatePastSubmissionData = async () => {
      try {
        ...
        const validResponse = await readFoodSecurityRecord(
          submissionId,
          user.id
        );
  ...
  const renderQuestion = (
    ...
    <>
      ...
      {watch(mainQuestion) === "Yes" && (
        <div>
          <label className="block mb-2">
            {questionNumber}.a How often did this happen?
          </label>
          <select
            {...register(subQuestion)}
            className="w-full p-2 border rounded"
          >
            <option value="Rarely (once or twice in the past four weeks)">
              Rarely (once or twice in the past four weeks)
            </option>
            <option value="Sometimes (three to ten times in the past four weeks)">
              Sometimes (three to ten times in the past four weeks)
            </option>
            <option value="Often (more than ten times in the past four weeks)">
  ...
  return (
    <div className="container mx-auto p-4 w-full h-full flex flex-col items-center">
      ...
      <form
        ...
        className="space-y-6 w-[40rem] md:w-[30rem] m-5 md:m-0 [&>p]:pt-6 [&>p]:pb-1 [&>input]:px-4"
      >
        {renderQuestion(
          "worryHouseholdWithoutFood",
          "howOftenWorryHouseholdWithoutFood",

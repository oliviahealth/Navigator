...
const LeftSidebar: React.FC = () => {
  ...
  const tabs = [
    { label: "Logs and Consent Forms", path: "/dashboard/logs-and-consent-forms" },
  ...
  return (
    <div className="w-full h-full my-5 px-5">
      {tabs.map(({ label, path }) => (
        <button
          key={label}
          className={`group w-full mb-3 p-2 gap-1 rounded-md flex items-center font-medium duration-300 transition-colors ${pathname === path ? "bg-maroon text-white" : "hover:bg-maroon/90 hover:text-white"
            }`}

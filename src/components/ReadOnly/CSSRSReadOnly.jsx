...
function CSSRSReadOnly() {
    ...
    useEffect(() => {
        const fetchLog = async () => {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/api/get_read_only_data/cssrs/${patientId}/${log_id}`, {
                    method: 'GET',
                    headers: {
                ...
                setIdeationResponses(data[2].ideationResponses);
                setIntensityResponses(data[2].intensityResponses);
                setBehaviorData(data[2].behaviorData);
                setLethalityData(data[2].lethalityData);

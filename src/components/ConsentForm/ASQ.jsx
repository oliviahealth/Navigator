...
function ASQ() {
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="form-section questionnaire">
        ...
        {/* Render checkboxes for each month interval */}
        {Array.from({ length: 21 }, (_, i) => i * 2).map(month => (
          <div key={month}>
            <input type="checkbox" id={`${month}month`} name="questionnaireUsed" value={month} onChange={handleInputChange} checked={formValues.questionnaireUsed === `${month}`} />
      ...
      <div className="form-section">
        ...
        <div>
          <input type="radio" id="yesPremature" name="adjustedForPrematurity" value="Yes" onChange={handleInputChange} checked={formValues.adjustedForPrematurity === 'Yes'} />
          <label htmlFor="yesPremature">Yes</label>
          <input type="radio" id="noPremature" name="adjustedForPrematurity" value="No" onChange={handleInputChange} checked={formValues.adjustedForPrematurity === 'No'} />
  ...
  <input type="text" id="communicationScore" name="communicationScore" onChange={handleInputChange} value={formValues.communicationScore} />

  <label htmlFor="grossMotorScore">Gross Motor Score:</label>
  <input type="text" id="grossMotorScore" name="grossMotorScore" onChange={handleInputChange} value={formValues.grossMotorScore} />

  <label htmlFor="fineMotorScore">Fine Motor:</label>
  <input type="text" id="fineMotorScore" name="fineMotorScore" onChange={handleInputChange} value={formValues.fineMotorScore} />

  <label htmlFor="problemSolvingScore">Problem Solving:</label>
  <input type="text" id="problemSolvingScore" name="problemSolvingScore" onChange={handleInputChange} value={formValues.problemSolvingScore} />

  <label htmlFor="personalSocialScore">Personal-Social:</label>
  <input type="text" id="personalSocialScore" name="personalSocialScore" onChange={handleInputChange} value={formValues.personalSocialScore} />
  ...
  <div className="checkbox-group">
    <label>
      <input type="checkbox" id="developmentalSupport" name="developmentalSupport" onChange={handleInputChange} checked={formValues.developmentalSupport} />
      Provide developmental support activities
    </label>

    <label>
      <input type="checkbox" id="rescreen" name="rescreen" onChange={handleInputChange} checked={formValues.rescreen} />
      Rescreen at next interval
    </label>

    <label>
      <input type="checkbox" id="referEarlySteps" name="referEarlySteps" onChange={handleInputChange} checked={formValues.referEarlySteps} />
      Refer to Early Steps or Child Find
    </label>

    <label>
      <input type="checkbox" id="referCommunityAgency" name="referCommunityAgency" onChange={handleInputChange} checked={formValues.referCommunityAgency} />
      Refer to other community agency/provider
    </label>

    <label>
      <input type="checkbox" id="noFurtherAction" name="noFurtherAction" onChange={handleInputChange} checked={formValues.noFurtherAction} />
  ...
  {formValues.developmentalSupport && (
    <div>
      <label htmlFor="describeActivities">Describe activities below. Check all that apply (required if developmental support activities are provided)</label>
      <textarea id="describeActivities" name="describeActivities" onChange={handleInputChange} value={formValues.describeActivities}></textarea>
    </div>
  )}
</div>

<label htmlFor="followUpNotes">Describe activities below (required if developmental support activities are provided)</label>
  <textarea id="followUpNotes" name="followUpNotes" onChange={handleInputChange} value={formValues.followUpNotes}></textarea>


      {/* Include other sections based on the image uploaded */}
